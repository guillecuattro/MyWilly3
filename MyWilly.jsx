...
Incluye entre 4 y 6 noticias. Busca eventos reales de hoy: geopolítica, bancos centrales, inflación, divisas, sectores clave. Relaciona cada noticia con los fondos específicos del usuario.`;
      ...
      setNewsError("No se pudieron cargar las noticias. Comprueba tu conexión.");
    }
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user",
            content: `Eres un presentador de radio financiero español. Reescribe el siguiente texto para que suene completamente natural al leerlo en voz alta, como lo haría un locutor profesional. Reglas:
- Expande TODAS las abreviaturas: EE.UU. → "Estados Unidos", BCE → "Banco Central Europeo", Fed → "Reserva Federal", IPC → "Índice de Precios al Consumo", PIB → "Producto Interior Bruto", ETF → "fondo cotizado", bps → "puntos básicos", etc.
- Los números: 4,25% → "cuatro coma veinticinco por ciento", 400.000 → "cuatrocientos mil", 1,4 → "uno coma cuatro"
- Los símbolos: € → "euros", $ → "dólares", % → "por ciento"
- Rangos: 4,25%–4,50% → "entre el cuatro coma veinticinco y el cuatro coma cincuenta por ciento"
- Frases naturales: no digas "el FOMC votó", di "la Reserva Federal decidió"
- Mantén el mismo significado y los mismos datos, solo cambia cómo suena
...
${text}`
      ...
      const data = await res.json();
      const spoken = data.content?.[0]?.text?.trim() || text;
  ...
  return(
    <>
      ...
      <div style={{minHeight:"100svh",background:C.bg,fontFamily:"'Plus Jakarta Sans',sans-serif",color:C.text,maxWidth:ip?820:430,margin:"0 auto",display:"flex",flexDirection:"column",paddingTop:"env(safe-area-inset-top, 0px)"}}>
        ...
        {/* CONTENT */}
        <div style={{flex:1,overflowY:"auto",padding:ip?"18px 22px 100px":`13px 13px calc(94px + env(safe-area-inset-bottom, 0px))`}}>
          {tab==="home"    &&<HomeView/>}
          {tab==="funds"   &&<FundView/>}
          {tab==="news"    &&<NewsView/>}
        ...
        {/* NAV — two rows, 3 tabs each */}
        <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:ip?820:430,background:C.surface,borderTop:`1px solid ${C.border}`,zIndex:50,paddingBottom:`env(safe-area-inset-bottom, 0px)`}}>
          {[
          ...
          {/* Row divider */}
          <div style={{position:"absolute",top:"44%",left:"5%",right:"5%",height:"1px",background:C.border,opacity:0.5}}/>
        </div>
        <Wizard/>
        <AddFundModal/>
        <ConfirmDeleteModal/>
